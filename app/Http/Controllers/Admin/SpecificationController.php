...
class SpecificationController extends VoyagerBaseController
{
    ...
    public function edit(Request $request, $id)
    {
        ...
        $listValues = ListValue::where('specification_id', $id)->get()->toArray();
        return Voyager::view($view, compact('dataType', 'dataTypeContent', 'isModelTranslatable', 'listValues'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        $specification->list=$request->has('list')?true:false;
        $specification->save();

        ListValue::where('specification_id',$id)->delete();

        if ($request->has('list')) {
            $listValues = [];
            foreach ($request->listValues as $key=>$value) {
                if (!empty($value) AND !is_null($value)) {
                    $listValues[$key]['specification_id'] = $id;
                    $listValues[$key]['name'] = $value;
                }
            }
            $specification->listValues()->createMany($listValues);
        }

        event(new BreadDataUpdated($dataType, $specification));
    ...
    public function store(Request $request)
    {
        ...
        $specification = Specification::create([
            'name' => $request->name,
            'list' => $request->has('list') ? true : false
        ]);
        if ($request->has('list')) {
            $listValues = [];
            foreach ($request->listValues as $i=>$value) {
                if (!empty($value) AND !is_null($value)) {
                    $listValues[$i]['specification_id'] = $specification->id;
                    $listValues[$i]['name'] = $value;
                }
            }
//            dd($listValues);
            $specification->listValues()->createMany($listValues);
        ...
        event(new BreadDataAdded($dataType, $specification));

        if (!$request->has('_tagging')) {
            if (auth()->user()->can('browse', $specification)) {
                $redirect = redirect()->route("voyager.{$dataType->slug}.index");
            } else {
                $redirect = redirect()->back();
            }
            ...
            return response()->json(['success' => true, 'data' => $specification]);
        }
    }
